...
export default function Stacy() {
  return (
    <div style={{ position: 'relative', width: 140, height: 240 }}>
      <Canvas concurrent shadowMap camera={{position: [1, 1.5, 1.5], fov: 50 }}>
        <ambientLight />
...
function Model(props) {
  ...
  const { color, scale } = useSpring({ scale: hovered ? [1.15, 1.15, 1] : [0.3, 0.3, 0.3], color: hovered ? 'hotpink' : 'aquamarine' });
